...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action === 'create_user') {
        ...
        } catch(Exception $e) { $error = "FAILED TO CREATE USER: ID/EMAIL ALREADY EXISTS."; }
    }
    ...
    elseif ($action === 'create_event') {
        ...
        $max_p = $_POST['max_participants']; $coord_id = $_POST['coordinator_id'] ?: null;
...
<div style="padding: 40px;">
    <div class="dashboard-header">
        ...
        <div class="header-actions">
            <span class="status-tag" style="background: rgba(168, 85, 247, 0.1); color: var(--secondary); padding: 8px 16px; font-size: 0.8rem; border-radius: 8px;">
    ...
    <?php if($msg): ?>
        <div style="margin-bottom: 20px; padding: 15px; background: rgba(16, 185, 129, 0.1); border: 1px solid var(--success); color: var(--success); border-radius: 12px; font-size: 0.9rem;">
            <i class="fa-solid fa-circle-check"></i> <?= $msg ?>
        </div>
    <?php endif; ?>
    <?php if($error): ?>
        <div style="margin-bottom: 20px; padding: 15px; background: rgba(239, 68, 68, 0.1); border: 1px solid var(--danger); color: var(--danger); border-radius: 12px; font-size: 0.9rem;">
    ...
    <div class="tab-container">
        <button onclick="showTab('dashboard')" class="tab-btn active" id="tab-dashboard"><i class="fa-solid fa-chart-pie" style="margin-right: 8px;"></i> DASHBOARD</button>
        <button onclick="showTab('users')" class="tab-btn" id="tab-users"><i class="fa-solid fa-users" style="margin-right: 8px;"></i> USER MANAGEMENT</button>
        <button onclick="showTab('events')" class="tab-btn" id="tab-events"><i class="fa-solid fa-calendar-alt" style="margin-right: 8px;"></i> EVENT MANAGEMENT</button>
        <button onclick="showTab('regs')" class="tab-btn" id="tab-regs"><i class="fa-solid fa-clipboard-check" style="margin-right: 8px;"></i> REGISTRATION DESK</button>
    </div>
    ...
    <div id="content-dashboard" class="admin-tab-content">
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; margin-bottom: 40px;">
            <div class="glass-panel-dash" style="padding: 30px; text-align: center;">
                <div style="font-size: 0.7rem; color: var(--text-dim); text-transform: uppercase;">Total Participants</div>
    ...
    <div id="content-users" class="admin-tab-content" style="display: none;">
        <div style="display: grid; grid-template-columns: 1fr 350px; gap: 30px;">
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <form method="POST">
                                        ...
                                        <select name="role" onchange="this.form.submit()" class="modern-select" style="padding: 6px 12px; width: auto; display: inline-block;">
                                ...
                                <td style="text-align: right;">
                                    <form method="POST" onsubmit="return confirm('Delete user?');">
                                        ...
                                        <input type="hidden" name="user_id" value="<?= $u['user_id'] ?>">
                                        <button type="submit" class="btn-icon-danger" title="Delete User"><i class="fa-solid fa-trash-can"></i></button>
                                    </form>
                                </td>
                            </tr>
                            <?php endforeach; ?>
            ...
            <div class="glass-panel-dash" style="padding: 30px;">
                <h3 style="font-family: 'Outfit'; font-size: 1.1rem; color: var(--primary); margin-bottom: 20px;">Onboard New User</h3>
                <form method="POST">
                    <input type="hidden" name="action" value="create_user">
                    <div class="form-group" style="margin-bottom: 15px;"><label class="modern-label">User ID / Reg No.</label><input type="text" name="user_id" class="modern-input" required placeholder="e.g. STD1001"></div>
                    <div class="form-group" style="margin-bottom: 15px;"><label class="modern-label">Full Name</label><input type="text" name="name" class="modern-input" required placeholder="John Doe"></div>
                    <div class="form-group" style="margin-bottom: 15px;"><label class="modern-label">Email Address</label><input type="email" name="email" class="modern-input" required placeholder="john@example.com"></div>
                    <div class="form-group" style="margin-bottom: 15px;"><label class="modern-label">Phone Number</label><input type="text" name="phone" class="modern-input" required placeholder="1234567890"></div>
                    <div class="form-group" style="margin-bottom: 15px;"><label class="modern-label">Role</label><select name="role" class="modern-select"><option value="student">Student / User</option><option value="coordinator">Coordinator</option><option value="admin">Administrator</option></select></div>
                    <div class="form-group" style="margin-bottom: 25px;"><label class="modern-label">Default Password</label><input type="password" name="password" class="modern-input" required placeholder="********"></div>
    ...
    <div id="content-events" class="admin-tab-content" style="display: none;">
        <div style="display: grid; grid-template-columns: 1fr 380px; gap: 30px;">
                    ...
                    <table class="modern-table">
                        ...
                        <tbody>
                            ...
                            <tr style="cursor: pointer;" onclick='editEvent(<?= json_encode($ev) ?>)'>
                                <td>
                                    <div style="font-weight: 600; color: var(--primary);"><?= htmlspecialchars($ev['name']) ?></div>
                                    <div style="font-size: 0.7rem; color: var(--text-dim); margin-top: 4px;"><i class="fa-solid fa-layer-group"></i> <?= $ev['category'] ?> Track</div>
                                </td>
                                <td style="text-align: center;"><i class="fa-solid fa-location-dot text-dim"></i> <?= $ev['venue'] ?: 'TBD' ?></td>
                                <td style="text-align: right;">
                                    <div style="font-weight: 600;"><?= htmlspecialchars($ev['coordinator_name'] ?: 'System Assigned') ?></div>
                                    <div style="font-size: 0.65rem; color: var(--text-dim);"><?= $ev['coordinator_id'] ?: 'AUTO' ?></div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
            ...
            <div class="glass-panel-dash" style="padding: 30px;">
                <h3 id="ev-form-title" style="font-family: 'Outfit'; font-size: 1.1rem; color: var(--primary); margin-bottom: 20px;">Deploy New Event</h3>
                <form id="ev-form" method="POST">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 15px;">
                        ...
                        <div><label class="modern-label">Max Cap</label><input type="number" name="max_participants" id="ev-max" value="50" class="modern-input"></div>
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom: 25px;"><label class="modern-label">Instructions / Rules (Markdown Supported)</label><textarea name="rules" id="ev-rules" class="modern-textarea" style="height: 100px; resize: vertical;" placeholder="1. Individual participation.\n2. Bring your own laptop."></textarea></div>
    ...
    <div id="content-regs" class="admin-tab-content" style="display: none;">
        <div class="glass-panel-dash" style="padding: 30px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
                ...
                <button onclick="window.print()" class="btn-coord" style="padding: 6px 16px; font-size: 0.7rem; border: 1px solid var(--primary); color: var(--primary);"><i class="fa-solid fa-file-pdf"></i> RECORD EXPORT</button>
            </div>
            <div style="overflow-x: auto;">
                <table class="modern-table">
                    <thead>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div style="font-weight: 600; font-size: 0.95rem;"><?= htmlspecialchars($r['user_name']) ?></div>
                                <div style="font-size: 0.7rem; color: var(--text-dim); margin-top: 4px;"><i class="fa-solid fa-graduation-cap"></i> <?= htmlspecialchars($r['college']) ?></div>
                            </td>
                            <td>
                                <div style="font-weight: 500; color: var(--secondary);"><?= htmlspecialchars($r['event_name']) ?></div>
                            </td>
                            <td style="text-align: center;">
                                <form method="POST">
                                    ...
                                    <?php 
                                        $statusClass = '';
                                        if($r['status'] == 'registered') $statusClass = 'pending';
                                        elseif($r['status'] == 'participated') $statusClass = 'approved';
                                        elseif($r['status'] == 'winner' || $r['status'] == 'runner') $statusClass = 'winner';
                                    ?>
                                    <select name="status" onchange="this.form.submit()" class="status-select <?= $statusClass ?>">
                                        <option value="registered" <?= $r['status'] == 'registered' ? 'selected' : '' ?>>PENDING APPROVAL</option>
                                        ...
                                        <option value="runner" <?= $r['status'] == 'runner' ? 'selected' : '' ?>>🥈 RUNNER-UP</option>
                                    </select>
                                </form>
                            </td>
                            <td style="text-align: right; font-size: 0.8rem; color: var(--text-muted);">
                                <i class="fa-regular fa-clock"></i> <?= date('d M, h:i A', strtotime($r['created_at'])) ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                        <?php if(empty($all_regs)): ?>
                            <tr><td colspan="4" style="padding: 60px; text-align: center; color: var(--text-dim);">No active registrations.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
...
.tab-btn { 
    background: none; border: none; color: var(--text-dim); padding: 12px 24px; 
    font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 0.85rem; 
    cursor: pointer; transition: all 0.3s ease; border-radius: 12px 12px 0 0; 
    position: relative; white-space: nowrap;
}
.tab-btn:hover { color: var(--text-main); background: rgba(255,255,255,0.02); }
.tab-btn.active { color: var(--primary); background: rgba(99, 102, 241, 0.05); }
.tab-btn.active::after { 
    content: ''; position: absolute; bottom: -5px; left: 0; 
    width: 100%; height: 3px; background: var(--primary); 
    border-radius: 10px; box-shadow: 0 0 10px rgba(99, 102, 241, 0.5);
}
...
.modern-input, .modern-select, .modern-textarea {
    ...
    color: var(--text-main); border-radius: 10px; font-family: 'Plus Jakarta Sans', sans-serif;
...
.btn-icon-danger:hover { background: var(--danger); color: white; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(239, 68, 68, 0.3); }
...
.status-select.pending { background-color: rgba(99, 102, 241, 0.1); color: var(--primary); border-color: rgba(99, 102, 241, 0.3); }
.status-select.approved { background-color: rgba(16, 185, 129, 0.1); color: var(--success); border-color: rgba(16, 185, 129, 0.3); }
.status-select.winner { background-color: rgba(245, 158, 11, 0.1); color: var(--warning); border-color: rgba(245, 158, 11, 0.3); }
.status-select:focus { box-shadow: 0 0 0 2px var(--bg-dark), 0 0 0 4px var(--primary); outline: none; }
</style>
